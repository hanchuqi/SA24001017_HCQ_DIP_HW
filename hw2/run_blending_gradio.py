...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = torch.tensor([[0, 1, 0], [1, -4, 1], [0, 1, 0]],
                                    dtype=torch.float32, device=foreground_img.device).repeat(3, 1, 1, 1)

    # 计算前景和混合图像的拉普拉斯特征
    foreground_laplacian = F.conv2d(foreground_img, laplacian_kernel, padding=1, groups=3)
    blended_laplacian = F.interpolate(F.conv2d(blended_img, laplacian_kernel, padding=1, groups=3), size=foreground_laplacian.shape[2:])
    # 计算拉普拉斯能量差并生成掩码区域
    energy = (foreground_laplacian - blended_laplacian).pow(2)
    background_mask_resized = F.interpolate(background_mask, size=foreground_mask.shape[2:])
    mask_new = foreground_mask.bool() & background_mask_resized.bool()
