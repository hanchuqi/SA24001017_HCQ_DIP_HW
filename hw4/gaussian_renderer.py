...
class GaussianRenderer(nn.Module):
    ...
    # 3. Compute the determinant of the covariance matrix
        ...
        dx=dx.unsqueeze(-1)
        cov_inv = cov_inv.unsqueeze(1).unsqueeze(2)  # (N, 1, 1, 2, 2)
        cov_inv = cov_inv.expand(-1, H, W, -1, -1)  # (N, H, W, 2, 2)
        # 5. Compute quadratic term (N, H, W)
        dx_cov_inv = torch.matmul(dx.transpose(3,4),torch.matmul(cov_inv,dx)).squeeze(-1).squeeze(-1) # (N, H, W)
        
        # 6. Compute the normalization factor for the Gaussian
        normalization = 1 / (2 * torch.pi * torch.sqrt(cov_det))  # (N, )
        
        # 7. Compute Gaussian values (N, H, W)
        gaussian = normalization.unsqueeze(-1).unsqueeze(-1) * torch.exp(-0.5 * dx_cov_inv)  # (N, H, W)
