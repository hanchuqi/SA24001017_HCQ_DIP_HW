...
class GaussianModel(nn.Module):
    ...
    # Compute covariance: Cov = R * S * S^T * R^T
        Covs3d = torch.bmm(R, S)  # Shape: (N, 3, 3), Matrix multiplication of R * S
        Covs3d = torch.bmm(Covs3d, S.transpose(1, 2))  # Shape: (N, 3, 3), Multiply by S^T
        Covs3d = torch.bmm(Covs3d, R.transpose(1, 2))  # Shape: (N, 3, 3), Multiply by R^T
